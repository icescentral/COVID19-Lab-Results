...
The input file for this script is a SAS dataset (.sasb7bdat) containing order IDs, lab names, test request codes, observation codes (LOINC), test result release times, test result statuses, and test result free-text. The output file of this script is a csv with test results (Positive [P]/Presumptive-positive [S]/Negative [N]/Indeterminate [I]/Pending [D]/Cancelled [C]/Rejected [R]) in multiple columns (one for each virus) and in the same order as the input dataset, along with an exclude_flag variable based on whether the test result was withdrawn.
